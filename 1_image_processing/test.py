...
def test_inverted_2():
    original = {
        'height': 1,
        'width': 4,
        'pixels': [11, 75, 132, 224],
    }
    result = lab.inverted(original)
    expected = {
        'height': 1,
        'width': 4,
        'pixels': [244,180,123,31],
    }
    compare_images(result, expected)
...
def test_blurred_centered_pixel():
    ...
    centered = lab.load_greyscale_image("test_images/centered_pixel.png")
    result = lab.blurred(centered, 3)
    expected = {
        'height': 11,
        'width': 11,
        'pixels': [
...
def test_correlate_wrap():
    ...
    result = lab.correlate(original, kernel, "wrap")
    expected = {
        'height': 3,
        'width': 2,
